...
export default function Header({ playerName, currentFirm, staff, currentRank, playerPosition, firmsCount, quarter, advanceQuarter }) {
  return (
    <header className="w-full px-6 py-5 flex items-center justify-between flex-wrap gap-3" style={{ borderBottom: `1px solid ${PALETTE.line}` }}>
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs mt-1" style={{ color: PALETTE.textMuted }}>{playerName} chez {currentFirm.name} · Sup. {staff.superior} · Rival {staff.rival}</p>
        </div>
      </div>
      <div className="flex items-center gap-4 text-sm">
        <div className="flex items-center gap-2"><Award size={16} color={PALETTE.gold} /><span style={{ color: PALETTE.gold }}>{currentRank.name}</span></div>
